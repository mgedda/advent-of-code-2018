...
object Day10_1
{
  ...
  def getCompactPointsRec(points: Seq[Point], boundingBox: BoundingBox) : Seq[Point] =
  {
    ...
    if (updatedBoundingBox.size > boundingBox.size)
      ...
      getCompactPointsRec(updatedPoints, updatedBoundingBox)
  }

  def printMessage(pointDefinitions: Seq[String]) : Boolean =
  {
    ...
    val compactPoints = getCompactPointsRec(points, getBoundingBox(points))

    printPoints(compactPoints)
